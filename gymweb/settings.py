...
CSRF_TRUSTED_ORIGINS = _cfg(
    ...
    default="http://127.0.0.1:5174,http://localhost:5174,http://test.gigxmatch.com:5174",
    cast=Csv(),
)
if isinstance(CSRF_TRUSTED_ORIGINS, str):
    CSRF_TRUSTED_ORIGINS = [s.strip() for s in CSRF_TRUSTED_ORIGINS.split(",") if s.strip()]
